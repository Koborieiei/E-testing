...
$pass = @$_REQUEST['pass'];
$type = @$_REQUEST['type'];
$type_login = @$_REQUEST['type_login'] == 'sso' ? "sso" : "";
$redri = "../../pages/home";

// echo 'username:' . $user . '<br>' . 'pass:' . $pass . '<br>' . 'type:' . $type . '</br>' . 'type-login:' . $type_login;
...
if ($json['status'] != 'fail') {
  ...
  $status  = 'success';
  $message = 'success';
  $wstoken = '9833bbe63c74be3f98eb84899fcc7425';
...
if ($status == 200 && $type_login == 'sso') {
  header("location: https://edbot.com/backbone/pages/home/");
  exit(0);
} else {
  header('Content-Type: application/json');
  exit(json_encode(array('status' => 'danger', 'message' => service('9833bbe63c74be3f98eb84899fcc7425', $data)[0], 'token' => $wstoken, "data" => $data)));
}
