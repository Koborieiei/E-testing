...
export default class runTestingPageClass {
 ...
 async _runTestingPage() {
  try {
   await this._storeAvailableQuestion()
   this.testingId = this.availableQuestion.testinginfo.id
   this._setupDoingTestingTime()
 ...
 _setTimeCountdown() {
  //   console.log(this.testingPageHeader.testingTimeleft)
 ...
 async _storeAvailableQuestion() {
  ...
  this.availableQuestion = await new Apiservice()._getQuestionObjects(
   testingRequestData
  )

  if (this.availableQuestion.message == 'Test is limited')
   throw 'คุณได้ทำบททดสอบครบตามจำนวนที่กำหนดไว้แล้ว'
 }
 ...
 _setupDoingTestingTime() {
  localStorage.setItem(
   'testingtime',
   this.availableQuestion.testinginfo.timeleft
  )
 }

 _displayErrorPage(error) {
  const msg =
   error == `Error: Question has not found` ? `ไม่พบคำถามในรายวิชานี้` : error
  document.getElementById(
   'app'
         ...
         <div class="mt-4">
                <button onclick="window.history.go(-1)" class="btn btn-lg btn-primary"> กลับหน้าแรก </button>
         </div>
